...
async def checkout_process(page):
    ...
    await page.click_locator('.btn-dark_grey.btn-checkoutAll.nclick')

    # You Have Promotions! 팝업
    await page.locator('button.btn-sure', has_text="Continue To Checkout").click()

    '''
    Checkout Step1_Shipping
    '''
    # Save & Continue 버튼 클릭
    await page.locator('.btn-dark_grey.btn-goToPayment').click()

    ## Verify Your Address 팝업
    await page.locator('.common-btn.c-black', has_text="Keep This Address").click()

    '''
    Checkout Step2_Payment
    '''
    # Save & Continue 버튼 클릭
    await page.click_locator('.btn-dark_grey.btn-goToReview')

    '''
    Checkout Step3_Order Review
    '''
    # Submit Order 버튼 클릭
    await page.click_locator('.btn-dark_grey.btn-checkout')

    # 잠시 대기 (3초 대기)
    await page.wait_for_timeout(3000)

    # 주문 완료 후 Thank you for your order! 텍스트가 포함된 h2 요소 확인
    await page.wait_for_load_state()  # 페이지가 완전히 로드될 때까지 기다리기
    if await page.locator('h2.order-title').count() > 0:  # h2 태그의 order-title 클래스가 1개 이상 있으면 성공
...
async def checkout_promotion(page):
    ...
    await page.click_locator('button.btn-vendor.size-medium_blue[data-nclick-extra*="vid=16502"]')
    # 60% Off & Free Shipping $50.00+ Orders
    apply_button = await page.locator('button.btn-apply.nclick', has_text="Apply").nth(0) # 첫 번째 버튼 클릭
    apply_button.click()
    ...
    # 주문 완료 후 Thank you for your order! 텍스트가 포함된 h2 요소 확인
    await page.wait_for_load_state()  # 페이지가 완전히 로드될 때까지 기다리기
    if await page.locator('h2.order-title').count() > 0:  # h2 태그의 order-title 클래스 1개 이상 있으면 성공
        print("Order successful! Test passed.")
    else:
        print("Order not found! Test failed.")